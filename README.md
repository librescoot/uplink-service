...
`navigate` sends a complete single destination (`latitude`, `longitude`, optional `address`) as `plan.replace` with one stop to the MDB settings-service RPC `settings:route-plan`. An optional nonempty `waypoints` array replaces it with multiple stops when the scooter advertises `nav=2`; entries accept `latitude`/`longitude` or `lat`/`lon` and optional `label`/`name`. An empty navigate command (or one without coordinates, waypoints, or a nonempty address) calls `plan.clear`. Partial destination commands, including an address without both coordinates, fail explicitly; send a complete coordinate pair instead. Owner RPC errors, including unavailability, produce a failed command response and never cause direct navigation writes. The diagnostic `redis` command also rejects `set`, `hset`, and `del` targeting the `navigation` key. The owner persists and publishes the resulting `navigation` projection; see [settings-service's route-plan contract](https://github.com/librescoot/settings-service#route-plan-rpc) for the wire schema.
